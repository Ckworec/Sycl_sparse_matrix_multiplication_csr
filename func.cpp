...
void sparse_matrix_multiply(const CSRMatrix& A, const CSRMatrix& B1, CSRMatrix &C, queue& q) {
    ...
    q.submit([&](handler& h) {
        ...
        h.parallel_for(range<1>(A.rows), [=](id<1> i) {
            ...
            /*for (int apos = 0; apos < A.non_zero_el;)
            {
                int r = acc_row_ptr_A[apos];

                for (int bpos = 0; bpos < B.non_zero_el;)
                {
                    ...
                    while (tempa < A.non_zero_el && acc_row_ptr_A[tempa] == r &&
                    ...
                    if (sum != 0)
                    {
                        acc_row_ptr_C.push_back(r);
                        acc_col_ind_C.push_back(c);
                        acc_values_C.push_back(sum);
                        nze++;
                    }

                    while (bpos < B.non_zero.el && acc_row_ptr_B[bpos] == c)
                        bpos++;
                }
                while (apos < A.non_zero.el && acc_row_ptr_A[bpos] == r)
                    apos++;
            }*/
        });
    }).wait();
